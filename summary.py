...
sentences = re.split('[?]', text)  # split text into sentences

# filter out sentences containing question mark
sentences_without_question = [s for s in sentences if "?" not in s]

text = ' '.join(sentences_without_question)
...
for i in range(int(math.floor(chunks_length)*0.7)):
    ...
    summary = summarizer_summary(
        chunks[i], max_length=max_len, min_length=max_len//2, do_sample=False)
